...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedUser?.isNotEmpty() == true && savedPassword?.isNotEmpty() == true) {
        ...
        btnIngresar.setOnClickListener {
            ...
            if (usuario.isNotEmpty() && contrasena.isNotEmpty()) {
                if (checkBoxRemember.isChecked) {
                    ...
                    acceder(usuario, contrasena, savedUser, savedPassword)
                } else {
                    acceder(usuario, contrasena, savedUser, savedPassword)
                }
            } else {
    ...
    private fun acceder(usuario: String, contrasena: String, savedUser: String?, savedPassword: String?) {
        if (usuario == savedUser && contrasena == savedPassword) {
            startActivity(Intent(this, ParquesActivity::class.java))
            finish()
        } else {
